...
from activemri.models.reconstruction_model_def import build_reconstruction_model
# from src.helpers.utils import build_optim
# from src.helpers import transforms


def load_recon_model(args, optim=False, is_LOUPE=False):
    if not is_LOUPE:
        checkpoint = torch.load(args.recon_model_checkpoint, map_location='cpu')
        recon_args = checkpoint['args']
        recon_model = build_reconstruction_model(recon_args)
        start_epoch = checkpoint['epoch']
    else:
        file = open(args.recon_model_checkpoint+"/args.pkl","rb")
        recon_args = pickle.load(file)
        file.close()
        print(recon_args)
        recon_args = DefaultMunch.fromDict(recon_args)
        recon_model = build_reconstruction_model(recon_args)
        checkpoint = torch.load(args.recon_model_checkpoint+"/best_recon.pt", map_location='cpu')
    ...
    if recon_args.data_parallel:  # if model was saved with data_parallel
        recon_model = torch.nn.DataParallel(recon_model, device_ids=[args.device])
