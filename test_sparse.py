...
from activemri.envs.sparse_vecenvs_kspace import SparseVecEnv_Kspace
import activemri.envs.ppo_vecenvs as ppo_vecenvs
...
def test(args):
    ...
    # env = SparseVecEnv_Kspace(args, mode='test')
    ...
    for _ in range(len(env._current_data_handler._data_loader)):
        ...
        # visualization
        try:
            os.mkdir(args.training_dir[:location]+'/choosing_visualization')
        except:
            pass
        for meta in metas:
            # if cnt % 500 == 0:
            # if cnt in [446,940,1182,1420,1644]:    # knee
            if cnt in [224,225,240,399,1742,1934,2496,2735]: 
                mask = torch.stack([env._current_mask[cnt%64,0]]*args.resolution)
                ...
                _save_image(mask, args.training_dir[:location]+'/choosing_visualization/mask_'+str(cnt+1))
                _save_image(kspace, args.training_dir[:location]+'/choosing_visualization/kspace_'+str(cnt+1))
                _save_image(recon_image, args.training_dir[:location]+'/choosing_visualization/recon_'+str(cnt+1))
                _save_image(gt, args.training_dir[:location]+'/choosing_visualization/gt_'+str(cnt+1))
    ...
    # save action 
    # location = args.training_dir.rfind('/')
    # np.save(args.training_dir[:location]+"/action_array.npy", action_array)
